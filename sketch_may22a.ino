...
#define JSON_BUFFER_SIZE 4096 // JSON处理缓冲区大小
...
const int SD_WRITE_INTERVAL = 600000;    // 写入间隔(ms)10分钟
...
void setup() {
  ...
  delay(1000);
  tft.init();           // 初始化TFT屏幕
  tft.setRotation(1);   // 屏幕方向（0-3),1为横向模式
...
void lcd_tft(){//TFT屏显示
  ...
  tft.setTextColor(TFT_WHITE,TFT_BLACK); //字白色背景黑色 
  tft.print("humidity :");
  tft.println(humidity);
  tft.print("random :"); tft.println(random(0,1000)); // 随机数
  tft.fillRect(0,200,135,25,TFT_GREEN);//绿色
  tft.setCursor(0, 200);//显示坐标
  tft.setTextColor(TFT_BLUE);//蓝色
